...
let sampleData = {}, ytPlayer, ytInterval, currentYTId = '', savedPass = '', isPlaying = false, okClicked = false; 
...
function getDirectImageUrl(url) {
    if (!url) return '';
    let fileId = '';
    if (url.includes('drive.google.com/uc?export=view&id=')) fileId = url.split('id=')[1].split('&')[0];
    else if (url.includes('drive.google.com/file/d/')) fileId = url.split('/file/d/')[1].split('/')[0];
...
async function init() {
    ...
    try {
        ...
        const dashRows = (await dashRes.text()).split('\n').map(row => row.split(','));
        let status = "CLOSED", ytId = "8RmZFUxos3E";
        ...
        try {
            ...
            const setRows = (await setRes.text()).split('\n').map(row => row.split(','));
        ...
        const dbRows = (await dbRes.text()).split('\n').map(row => row.split(','));
        sampleData = {};
    ...
    } catch (e) { catView.innerHTML = `<div>Lỗi kết nối dữ liệu.</div>`; }
}
...
async function updateToSheet() {
    ...
    const finalYtId = extractYouTubeId(document.getElementById('cc-yt').value.trim());
    let driveLinks = [];
    ...
    try {
        await fetch(WEB_APP_URL, {
            ...
            body: JSON.stringify({ password: savedPass, status: status, ytId: finalYtId, driveLinks: driveLinks })
        });
...
function toggleFolder(type) {
    const top = document.getElementById('folder-top'), bottom = document.getElementById('folder-bottom');
    if (type === 'top') { top.classList.toggle('open'); bottom.classList.remove('open'); } 
    else { bottom.classList.toggle('open'); top.classList.remove('open'); }
}
...
function addDriveInput(type = '', url = '') {
    ...
    div.innerHTML = `<input type="text" class="t-input" placeholder="Loại" value="${type}"><input type="text" class="l-input" placeholder="Link" value="${url}"><button onclick="this.parentElement.remove()">X</button>`;
    document.getElementById('drive-links-container').appendChild(div);
}
function initYoutube(id) {
    ...
    ytPlayer = new YT.Player('yt-hidden-player', {
        height: '0', width: '0', videoId: id, playerVars: { 'autoplay': 0, 'loop': 1, 'playlist': id },
        events: { 'onReady': onPlayerReady, 'onStateChange': onPlayerStateChange }
    });
}
function onPlayerReady(event) { if (okClicked) event.target.playVideo(); }
function onPlayerStateChange(event) {
    const playIcon = document.getElementById('play-pause-icon'), coverImg = document.getElementById('yt-cover'), titleDiv = document.getElementById('yt-title');
...
function updateProgressBar() {
    if (ytPlayer && ytPlayer.getDuration) document.getElementById('yt-progress').style.width = ((ytPlayer.getCurrentTime() / ytPlayer.getDuration()) * 100) + '%';
}
...
function toggleMute() {
    ...
    document.getElementById('mute-icon').className = isMuted ? "fas fa-volume-up" : "fas fa-volume-mute";
    isMuted = !isMuted;
}
function renderCategoryCards() {
    ...
    Object.keys(sampleData).forEach(category => {
        const card = document.createElement('div'); card.className = 'category-card'; card.onclick = () => openGallery(category, category);
        const bgGrid = document.createElement('div'); bgGrid.className = 'card-bg-grid';
        sampleData[category].slice(0, 4).forEach(url => {
            const thumb = document.createElement('div'); thumb.className = 'card-thumb'; thumb.style.backgroundImage = `url('${url}')`;
            bgGrid.appendChild(thumb);
        });
        const overlay = document.createElement('div'); overlay.className = 'card-overlay';
        const span = document.createElement('span'); span.innerText = category;
...
function openGallery(categoryKey, title) {
    ...
    sampleData[categoryKey].forEach((url, i) => {
        const div = document.createElement('div'); div.className = 'sample-item'; div.style.animationDelay = `${i * 0.1}s`;
